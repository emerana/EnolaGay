...
public class JudyFile: NSObject {
    ...
    public func jsonFormFile(fileName: String, fileType: String = "json") -> JSON {
        guard fileName.clean() != "" else {
            return JSON([EMERANA.Key.Api.error: [EMERANA.Key.Api.msg: "请传入文件名"]])
        }
        var json = JSON()
        //读取Json数据
        do {
            ...
            guard path != nil else {
                return JSON([EMERANA.Key.Api.error: [EMERANA.Key.Api.msg: "没有找到文件：\"\(fileName).\(fileType)\""]])
            }
            ...
            Judy.log("读取文件发生异常")
            json = JSON([EMERANA.Key.Api.error: [EMERANA.Key.Api.msg: "读取文件发生异常", "file": fileName]])
        }
