...
/**
 An HoshiTextField is a subclass of theJudyTextFieldEffects object, is a control that displays an UITextField with a customizable visual effect around the lower edge of the control.
 */
@IBDesignable open class JudyHoshiTextField: JudyTextFieldEffects {
    ...
    */
    ...
    /// 上下间距，默认为 6
    @IBInspectable dynamic var 上下间距: CGFloat = 6
    @IBInspectable dynamic var 占位间距: CGFloat = 2
    
    // 边界厚度
    fileprivate let borderThickness: (active: CGFloat, inactive: CGFloat) = (active: 2, inactive: 2)
    
    fileprivate var placeholderInsets: CGPoint {
        get{
            return CGPoint(x: 0, y: 占位间距)
        }
    }
    /// 原来这里可以修改占位符和text的上下间距
    fileprivate var textFieldInsets: CGPoint {
        get{
            return CGPoint(x: 0, y: 上下间距)
        }
    }
    ...
    fileprivate func layoutPlaceholderInTextRect() {        
        ...
        placeholderLabel.frame = CGRect(x: originX, y: textRect.height/2 - 6,
