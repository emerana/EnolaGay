...
@class JudySegmentedCtrl;

typedef void (^IndexChangeBlock)(NSInteger index);
typedef NSAttributedString *(^HMTitleFormatterBlock)(JudySegmentedCtrl *segmentedControl, NSString *title, NSUInteger index, BOOL selected);
...
/**
 - swift 使用方法: Copy 以下代码
 
 ```
 /// 生成滚动标题
 private func generateScrollTitleView(){
 // Segmented control with scrolling
 segmentedCtrl.sectionTitles = ["美食类", "书籍类", "服装美容类", "小家电类", "居家类", "测试类", "开发类", "抽象类"]
 // Judy-mark: 设置字体及颜色,默认未选中的字体颜色
 segmentedCtrl.titleTextAttributes = [NSAttributedStringKey.foregroundColor: Judy.colorByRGB(rgbValue: 0x666666), NSAttributedStringKey.font: UIFont(name: "PingFangSC-Regular", size: 16)as Any]
 
 segmentedCtrl.selectedSegmentIndex = 0
 segmentedCtrl.segmentWidthStyle = .dynamic
 //    segmentedControl1.segmentEdgeInset = UIEdgeInsetsMake(0, 0, 0, 0);
 //    segmentedControl1.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleWidth;
 
 segmentedCtrl.selectionStyle = .textWidthStripe
 segmentedCtrl.selectionIndicatorLocation = .down
 // 线条颜色
 segmentedCtrl.selectionIndicatorColor = Judy.colorByRGB(rgbValue: 0x22D784)
 segmentedCtrl.selectedTitleTextAttributes = [NSAttributedStringKey.foregroundColor: segmentedCtrl.selectionIndicatorColor]
 // 线条高度
 segmentedCtrl.selectionIndicatorHeight = 2
 // 线条的宽度通过偏移量设置，右边 = 左边 + 10
 segmentedCtrl.selectionIndicatorEdgeInsets = UIEdgeInsetsMake(0, 8, 0, 18)
 segmentedCtrl.addTarget(self, action: #selector(segmentedControlChangedValue(segmentedControl:)), for: .valueChanged)
