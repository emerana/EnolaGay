...
@property (nonatomic, strong) Y_KLineView *kLineView;/**< K线图View */

@property (nonatomic, strong) Y_StockChartSegmentView *segmentView;/**< 底部选择View */

@property(nonatomic,assign) Y_StockChartCenterViewType currentCenterViewType;/**< 图表类型 */

@property(nonatomic,assign,readwrite) NSInteger currentIndex;/**< 当前索引 */

@end


@implementation Y_StockChartView {
    NSUInteger defautlIndex;/**< 默认索引 */
}
...
- (void)setItemModels:(NSArray *)itemModels {
    ...
    if(itemModels) {
        ...
        self.segmentView.items = items; // 设置当前面板的titles
        Y_StockChartViewItemModel *firstModel = itemModels.firstObject;
        self.currentCenterViewType = firstModel.centerViewType; // 将第一个设为当前的视图类型
    }
    if(self.dataSource) {
