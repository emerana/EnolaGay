...
class TableController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func deleteCell(_ sender: Any) {
        if cellNumbers <= 0 {
           cellNumbers += 1
            self.tableView.insertRows(at:[IndexPath.init(row: 0, section: 0)] , with: UITableView.RowAnimation.fade)
        } else {
            cellNumbers -= 1
            self.tableView.deleteRows(at:[IndexPath.init(row: 0, section: 0)] , with: UITableView.RowAnimation.fade)
        }
    }
