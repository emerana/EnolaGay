...
/// 我的定投界面
class InvestmentViewCtrl: JudyBaseTableRefreshViewCtrl {
    ...
    private var fundList = [FundPurchased]()
    ...
    override func setApi() {
        ...
        fundList = FundDataSourceCtrl.judy.getInvestmentList()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "showFundDetailViewCtrl" {
            ...
            viewCtrl.fund = (sender as? FundPurchased)?.fund
            viewCtrl.deleteInvestmentCallback = {
                self.reqApi()
            }
        }
...
extension InvestmentViewCtrl {
    
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        (view.viewWithTag(1001) as? JudyBaseLabel)?.text = "\(fundList.count)只已购/定投基金"
        return fundList.count
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        // 此方法可以不判断 cell == nil
        let cell: InvestmentCell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! InvestmentCell
        cell.setFundPurchased(fundPurchased: fundList[indexPath.row])
